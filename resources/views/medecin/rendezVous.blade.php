...
<body class="bg-gray-200 flex justify-center items-center h-screen">
    <form class="w-full max-w-lg bg-white rounded-lg border border-gray-300 shadow-md p-6" action="{{ route('save-appointment') }}" method="POST">
            ...
            <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state" name="time">
                @foreach($times as $time)
                    ...
                    @foreach($rendezVous as $rdv)
                        @if($rdv->created_at >= $date_8h && $rdv->created_at <= $dateFormat && $rdv->date == $time)
                            <?php $isReserved = true; ?>
                            @break
                        @endif
                    @endforeach
                    <option value="{{$time}}" @if($isReserved) disabled style="color: red;" @endif>
                        {{$time}}
                        @if($isReserved)
                            réservé
                        @endif
        ...
        <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded mt-5">Submit</button>
    </form>
</body>
</html>
